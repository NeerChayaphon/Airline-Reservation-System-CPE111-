...
int linkFlight(FLIGHT_T *pFirstFlight, FLIGHT_T *pSecondFlight);
...
/*void findTransitFlight(ADJACENT_T * pEdge)
{
    pTransitFLight = (TRANSIT_T*) calloc(1,sizeof(pTransitFLight));
        
    if (pTransitFLight == NULL)
        {
            printf("Error\n");
            exit(1);
        }
        
    pTransitFLight->flight = pEdge->flightHead;
        
        if (pEdge->transitHead == NULL)
        {
            pEdge->transitHead = pTransitFLight;
        }
        else
        {
            pEdge->transitTail->next = pTransitFLight;
        }
        pEdge->transitTail = pTransitFLight;
}*/
...
void printFlight(VERTEX_T *pEndVertex)
{
    VERTEX_T **pathVertices = calloc(maxGraphVertices, sizeof(VERTEX_T *));
    /* this array is big enough to hold all the vertices we have */
    int pathCount = 0;
    ...
    else
    {
        ...
        /* Now start at the end of the array to print the path */
        for (i = pathCount - 1; i >= 1; i--)
        {
            printf("%d\n", findEdge(pathVertices[i]->key, pathVertices[i - 1]->key));
        }
...
{
    int bOk = 1;
    TRANSIT *pNewRef = NULL;
    
        pNewRef = (TRANSIT *)calloc(1, sizeof(TRANSIT));
        ...
        {
            pNewRef->flight = pSecondFlight;
            
            if (pFirstFlight->transitTail != NULL)
            {
                pFirstFlight->transitTail->next = pNewRef;
            }
            else
            {
