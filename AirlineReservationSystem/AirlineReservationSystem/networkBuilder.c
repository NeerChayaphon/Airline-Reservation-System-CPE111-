 ...
 *  Modified 19 March 2016 to ignore duplicate edges for an
 *  undirected network.
 */
...
void processCommand(char* command, int bVerbose,int bDirected);
...
int readNetworkDefinition(char* filename, int bVerbose)
{
    ...
    if (pFp == NULL)
       ...
       if (fgets(inputline,sizeof(inputline),pFp) != NULL)
		   ...
		   processCommand(inputline,bVerbose,bDirected);
		   }
               }
           else
               {
 ...
 *   bDirected - If true, this is a directed network, else undirected
 */
void processCommand(char* command, int bVerbose, int bDirected)
{
    char keystring1[64] = "";
