 ...
 *  Modified 19 March 2016 to add Prim's back, and include
 *  options for testing the write function
 */
...
int getMenuOption()
{
    ...
    while (option < 0)
       ...
       printf(" 10 - Write current network contents to a file\n");
       printf(" 11 - Exit\n");
       printf("Which option? ");
       fgets(input,sizeof(input),stdin);
       ...
       if ((option > 11) || (option < 1))
           {
	   printf("Invalid selection - choose 1 to 11\n");
	   option = -1;
	   }
       }
    printf("\n");
