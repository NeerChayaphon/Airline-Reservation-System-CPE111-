...
VERTEX_T *findVertexByKey(char *key, VERTEX_T **pPred)
{
    ...
    *pPred = NULL;
    ...
    {
        if (strcasecmp(pCurVertex->key, key) == 0)
        {
...
void findFlight(char *key1, char *key2, ADJACENT_T *pNewRef)
{
    ...
    char *pDeparture = NULL;
    char *pArrival = NULL;
    char *pFlightID = NULL;
    char *pDepartureHour = NULL;
    char *pDepartureMinute = NULL;
    char *pArrivalHour = NULL;
    char *pArrivalMinute = NULL;
    char *pAirport1 = NULL;
    char *pAirport2 = NULL;
    ...
    {
        sscanf(inputline, "%s %s %s %s %s %s %s %s %s %d", departure, arrival, flightID, departureHour, departureMinute, arrivalHour, arrivalMinute, airport1, airport2,&dayOfMonth);

        if ((strcmp(key1, departure) == 0) && (strcmp(key2, arrival) == 0))
        {
            pDeparture = strdup(departure);
            pArrival = strdup(arrival);
            pFlightID = strdup(flightID);
            pDepartureHour = strdup(departureHour);
            pDepartureMinute = strdup(departureMinute);
            pArrivalHour = strdup(arrivalHour);
            pArrivalMinute = strdup(arrivalMinute);
            pAirport1 = strdup(airport1);
            pAirport2 = strdup(airport2);

            pFlight = (FLIGHT_T *)calloc(1, sizeof(FLIGHT_T));
            pFlight->departure = pDeparture;
            pFlight->arrival = pArrival;
            pFlight->flightID = pFlightID;
            pFlight->departureHour = pDepartureHour;
            pFlight->departureMinute = pDepartureMinute;
            pFlight->arrivalHour = pArrivalHour;
            pFlight->arrivalMinute = pArrivalMinute;
            pFlight->airport1 = pAirport1;
            pFlight->airport2 = pAirport2;
            pFlight->dayOfMonth = dayOfMonth;
            ...
            {
                pNewRef->flightTail->next = pFlight;
            }
...
int addEdge(char *key1, char *key2, unsigned int weight)
{
    ...
    else
    {
        ...
        else
        {
            ...
            pCurrent = pNewRef->flightHead;
            while (pCurrent != NULL)
            {
                printf("|%s| -> |%s| : |%s| |%s:%s| - |%s:%s| %d\n", key1, key2, pCurrent->flightID, pCurrent->departureHour, pCurrent->departureMinute, pCurrent->arrivalHour, pCurrent->arrivalMinute,pCurrent->dayOfMonth);
...
void flightInpath(VERTEX_T *pEndVertex,int day)
{
    ...
    else
    {
        ...
        checkflightInDate(pathVertices,pathCount,day);
        
        free(pathVertices);
    }
}
void empty(VERTEX_T *pVertex)
{
}
...
int checkReachableFlight(char *key1, char *key2)
{
    int bOk = 1;
    VERTEX_T *pDummy = NULL;
    VERTEX_T *pStart = findVertexByKey(key1, &pDummy); /* find the starting node */
    VERTEX_T *pEnd = findVertexByKey(key2, &pDummy);   /* find the ending node */
    colorAll(WHITE);                                   /* mark all white */
    if ((pStart == NULL) || (pEnd == NULL))            /* check if exist or not */
    {
        bOk = -1;
    }
    else
    {
        traverseBreadthFirst(pStart, empty); /* check if exist or not */
        if (pEnd->color != BLACK)            /* check from black one mean that are connect */
        {
            bOk = 0;
        }
    }
 ...
 */
int printShortestPath(char *startKey, char *endKey,int day)
{
    int pathWeight = 0;
    VERTEX_T *pDummy = NULL;
